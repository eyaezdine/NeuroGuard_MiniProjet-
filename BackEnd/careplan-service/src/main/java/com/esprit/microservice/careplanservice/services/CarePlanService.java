...
import com.esprit.microservice.careplanservice.feign.MedicalHistoryServiceClient;
...
public class CarePlanService {
    ...
    private Priority priorityFromClinicalStage(ClinicalSummaryDto summary, String fallback) {
        if (summary == null || !summary.isHistoryFound() || summary.getProgressionStage() == null) {
            return parsePriority(fallback);
        }

        switch (summary.getProgressionStage().toUpperCase()) {
            case "SEVERE":
    ...
    public CarePlanResponse createCarePlan(CarePlanRequest request) {
        ...
        ClinicalSummaryDto clinicalSummary = medicalHistoryServiceClient.getClinicalSummary(request.getPatientId());

        CarePlan carePlan = new CarePlan();
        carePlan.setPatientId(request.getPatientId());
        carePlan.setProviderId(providerId);
        carePlan.setPriority(priorityFromClinicalStage(clinicalSummary, request.getPriority()));
